...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            ...
            + 'TEL;type=CELL;type=VOICE;waid=62895355566000:+62 895-3555-66000\n' // JANGAN KEK BABI SU
            + 'END:VCARD'
prefix = '.'
blocked = []
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["62895355566000@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["62895355566000@s.whatsapp.net"]
			const frendsowner = ["62895355566000@s.whatsapp.net"]
			...
			case 'owner':
                 ...
                 client.sendMessage(from, 'Itu Owner Bot SAD Ya Gans <-',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    reply('MASALAH ANDA TELAH DI LAPORKAN KE OWNER.')
                    break
                case 'request':
                    ...
                    client.sendMessage('+62895355566000@s.whatsapp.net', options, text, {quoted: mek})
                    reply('TERIMA KASIH UDAH REQUESTS KAKAK.')
                    break
				case 'meme':
               ...
               case 'infogempa':
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('SALAH KOMEN OM')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('UDAH AKTIF KAK')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('BERHASIL MENYALAKAN FITUR WELCOME✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('BERHASIL MENONAKTIFKAN FITUR WELCOME✔️')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Mohon Ditag Orangnya Kak')
